...
The following example shows that it is possible to perform calculations with
...
1-2*E(4)
\end{lstlisting}
%For the second exercise we see that the quotient is an element of $\Q[i]$ and not an element of $\Z[i]$, so
%$1+3i\nmid 4+i$: 
%\begin{lstlisting}
%gap> (4+i)/(1+3*i);
%7/10-11/10*E(4)
%gap> EuclideanRemainder(4+i,1+3*i); 
%-E(4)
%\end{lstlisting}
